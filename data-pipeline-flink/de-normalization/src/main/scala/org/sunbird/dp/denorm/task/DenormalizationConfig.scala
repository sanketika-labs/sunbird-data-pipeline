...
class DenormalizationConfig(override val config: Config, jobName: String) extends BaseJobConfig(config, jobName ) {
  ...
  val userFields = List("usertype", "usersubtype","grade", "language", "subject", "state", "district", "usersignintype", "userlogintype", "block", "cluster", "schoolname", "orgname","rootorgid", "usertype", "firstname", "lastname", "profileConfig", "email")
